...
if __name__ == "__main__":
    ...
    space_lim = (np.array(space_dim[:-1])-2)

    # Plot the input data
    if PLOT_FIELD_FIRST:
        ax = plt.figure().add_subplot(projection='3d')
        field_temp = field[:-2, :-2, :-2, :]
        field_temp = np.transpose(field_temp, (3, 0, 1, 2))
        x, y, z = field_loc[:, :-2, :-2, :-2]
        ax.quiver(x, y, z,
                  field_temp[0], field_temp[1], field_temp[2],
                  length = 1, normalize = True)
        plt.xlim(0, space_lim[0])
        plt.ylim(0, space_lim[1])
        ax.set_zlim(0, space_lim[2])
        plt.show()
